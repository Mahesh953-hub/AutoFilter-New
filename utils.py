...
async def format_fsub_status(bot, fsub_data: list):
    if not fsub_data:
        return "❌ No ForceSub data found in database."

    text = "📊 **Force-Sub Channel Summary**\n\n"

    for entry in fsub_data:
        name = entry["_id"]  # e.g. "FSUB"
        data = entry.get(name, {})
        channels = data.get("channels", [])
        last_updated = data.get("last_updated")

        if not channels:
            text += f"**{name}** → No channels found.\n\n"
            continue

        text += f"🌐 **{name}**  —  (Last updated: `{last_updated.strftime('%Y-%m-%d %H:%M:%S UTC')}`)\n"

        for ch in channels:
            cid = int(ch["_id"])
            invite_link = ch.get("invite_link") or "None"
            exp = ch.get("exp")
            set_by = ch.get("set_by")

            # Format datetime
            exp_fmt = exp.strftime("%Y-%m-%d %H:%M:%S UTC") if exp else "Unknown"

            try:
                chat = await bot.get_chat(cid)
                title = chat.title or "No Title"
            except ChatAdminRequired:
                title = "⚠️ Bot not admin"
