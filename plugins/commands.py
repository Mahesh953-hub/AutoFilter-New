...
from info import *
from utils import get_settings, save_group_settings, is_subscribed, is_req_subscribed, get_size, get_shortlink, is_check_admin, temp, get_readable_time, get_time, generate_settings_text, log_error, clean_filename, format_fsub_status
import time
...
#@Client.on_message(filters.private & filters.command("fuck") & filters.user(ADMINS))
@Client.on_message(filters.private & filters.command("fsub") & filters.user(ADMINS))
async def fsub_handler(client: Client, message: Message):
    """
    /fsub                  -> show usage
    /fsub get              -> print current FSUB status
    /fsub FSUB <id1> ...   -> set FSUB group with given channels
    """
    userid = message.from_user.id if message.from_user else None

    # split into parts: command, maybe name/get, maybe list
    parts = message.text.strip().split(maxsplit=2)  # limit 3 parts
    ...
    if len(parts) == 1:
        return await message.reply_text(
            ...
            "/fsub get\n"
            "/fsub FSUB <id1> <id2> ...\n\n"
            "Example: /fsub FSUB -1001234567890 -1009876543210</b>",
            disable_web_page_preview=True
        )
    ...
    if len(parts) >= 2 and parts[1].lower() == "get":
        try:
            data = await force_db.get_all()
            formatted = await format_fsub_status(client, data)
            return await message.reply_text(formatted, disable_web_page_preview=True)
        except Exception as e:
            return await message.reply_text(f"❌ Failed to fetch FSUB data:\n<code>{e}</code>")

    # Otherwise expect: /fsub <NAME> <ids...>
    if len(parts) < 3:
        return await message.reply_text(
            "<b>Incomplete command.\nUsage: /fsub FSUB <id1> <id2> ...</b>",
            disable_web_page_preview=True
        )
    ...
    # parse channel tokens (separated by whitespace)
    tokens = ids_part.split()
    if not tokens:
        return await message.reply_text(
            "<b>No channel IDs provided.\nUsage: /fsub FSUB <id1> <id2> ...</b>"
        )

    proc_msg = await message.reply_text("⏳ Processing channels...")

    valid_channel_ids = []
    failures = []

    # try to resolve each token via get_chat
    for tok in tokens:
        try:
            # try get_chat — accepts -100.. ids or @username or invite link
            chat = await client.get_chat(tok)
            # ensure it's a channel or supergroup (you asked channels)
            if chat.type not in (enums.ChatType.CHANNEL, enums.ChatType.SUPERGROUP):
                failures.append((tok, "Not a channel/supergroup"))
                continue
            # store id as int or string depending on your DB expectations
            cid = int(chat.id)
            valid_channel_ids.append(cid)
        except Exception as e:
            # record error, continue
            failures.append((tok, repr(e)))
            continue

    # nothing valid
    if not valid_channel_ids:
        await proc_msg.edit(
            f"❌ No valid channels found.\nFailures:\n" +
            "\n".join(f"- `{t}` : {reason}" for t, reason in failures),
            disable_web_page_preview=True
        )
        return

    # get current timeout from DB (fallback default inside get_fsub_timeout)
    try:
        timeout = await force_db.get_fsub_timeout()
    except Exception:
        timeout = None  # fallback to function default

    # Save to DB (expires_in uses timeout if available)
    expires_in = int(timeout) if timeout else 3600

    try:
        # set_fsub expects List[int] for channel_ids
        await force_db.set_fsub(name, valid_channel_ids, userid, expires_in=expires_in)
    except Exception as e:
        return await proc_msg.edit(f"❌ Failed to save FSUB to DB:\n<code>{e}</code>")

    # build reply
    ok_count = len(valid_channel_ids)
    fail_count = len(failures)
    lines = [f"✅ Saved {ok_count} channel(s) to `{name}` (expires in {expires_in} seconds)."]
    if fail_count:
        lines.append(f"⚠️ Failed to process {fail_count} token(s):")
...
@Client.on_message(filters.private & filters.command("set_ftime") & filters.user(ADMINS))
async def fsub_time(client, message):
    try:
        args = message.text.split(maxsplit=2)
        if len(args) != 2:
            return await message.reply_text("This command need an argument!\n\nUse `get` to get the current time\n`/set_fsub <time>` where <time> is seconds!")
        undefined = args[1]
        _type = type(undefined)
        if undefined == "get":
            data = await force_db.get_fsub_timeout()
            return await message.reply_text(f"currently fsub timeout is set as : {data}")
        if int(undefined):
            await force_db.set_fsub_timeout(undefined)
            await message.reply_text(f"Sucess! the timer has been updated with new value: {undefined}")
    except Exception as e:
        return await message.reply_text("Unexpected error! use /set_ftime to know more!")
